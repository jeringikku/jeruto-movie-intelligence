...
export default function PeopleIntelligenceBanner() {
  ...
  useEffect(() => {
    async function loadBanner() {
      const { data, error } = await supabase
        ...
        .eq("asset_key", "people_intelligence_banner")
        .eq("is_active", true)
        .limit(1)
        .maybeSingle();
      ...
      if (error) {
        console.error(
          "People Intelligence banner error:",
          error
        );
        setBanner(null);
      } else {
  ...
  if (loading) {
    return (
      <div className="mt-7 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950">
        <div className="aspect-[18/10] w-full animate-pulse bg-zinc-900/60" />
      </div>
    );
  }
