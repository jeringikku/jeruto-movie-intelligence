...
async function fetchStateBoxOfficeInChunks(
  ...
  for (let i = 0; i < movieIds.length; i += chunkSize) {
    ...
    if (error) {
      console.error(
        "Tamilnadu Tamil box-office query error:",
        error
      );
      continue;
    }
...
export default async function TamilNaduKannadaPage({
  searchParams,
}: Props) {
  ...
  if (moviesError) {
    console.error(
      "Tamilnadu Tamil movie query error:",
      moviesError
    );
  }
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <section className="mt-8 rounded-xl border border-zinc-900 bg-zinc-950/40 p-5">
          ...
          <p className="mt-2 text-[9px] leading-5 text-zinc-500">
            ...
            collection reported for Tamilnadu.
          </p>
        ...
        <div className="mt-7 flex justify-between border-t border-zinc-900 pt-5">
          ...
          <Link
            href="/preview/records/highest-grossing/state-wise/tamil-nadu/language-wise/english"
            className="text-[9px] text-violet-500 transition hover:text-zinc-300"
          >
            English →
          </Link>
