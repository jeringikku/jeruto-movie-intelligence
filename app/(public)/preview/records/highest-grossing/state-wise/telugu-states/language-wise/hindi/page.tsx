...
async function fetchStateBoxOfficeInChunks(
  ...
  for (let i = 0; i < movieIds.length; i += chunkSize) {
    ...
    if (error) {
      console.error(
        "Telugu States Hindi box-office query error:",
        error
      );
      continue;
    }
...
export default async function TeluguStatesHindiPage({
  searchParams,
}: Props) {
  ...
  if (moviesError) {
    console.error(
      "Telugu States Hindi movie query error:",
      moviesError
    );
  }
    ...
    <div className="min-h-screen bg-black text-zinc-300">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mt-7 flex justify-between border-t border-zinc-900 pt-5">
          ...
          <Link
            href="/preview/records/highest-grossing/state-wise/telugu-states/language-wise/kannada"
            className="text-[9px] text-violet-400 transition hover:text-zinc-300"
          >
            Kannada →
          </Link>
