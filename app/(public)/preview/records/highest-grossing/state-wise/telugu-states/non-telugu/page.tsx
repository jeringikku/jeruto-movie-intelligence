...
async function fetchStateBoxOfficeInChunks(
  ...
  for (
    ...
    if (error) {
      console.error(
        "Telugu States box-office query error:",
        error
      );
      continue;
    }
...
export default async function TeluguStatesNonTeluguPage({
  searchParams,
}: Props) {
  ...
  if (moviesError) {
    console.error(
      "Telugu States movie query error:",
      moviesError
    );
  }
  ...
  /*
   * =========================================================
   * 3. GET PRIMARY LANGUAGE MAPPING
   * =========================================================
   *
   * IMPORTANT:
   * Only the movie's primary language determines
   * whether it belongs to the Non-Telugu category.
   *
   * Secondary / dubbed Telugu associations are ignored.
   */

  let nonTeluguMovieIds =
    new Set<number>();
  ...
  if (movieIds.length > 0) {
    ...
    for (
      const row of primaryLanguageRows || []
    ) {
    ...
    if (languageIds.length > 0) {
      ...
      if (languageError) {
        console.error(
          "Language name query error:",
          languageError
        );
      }

      for (
        const language of languageRows || []
      ) {
        languageNameMap.set(
          Number(language.id),
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <section className="mt-8 rounded-xl border border-zinc-900 bg-zinc-950/40 p-5">
          ...
          <p className="mt-2 text-[9px] leading-5 text-zinc-400">
            ...
            primary language is Telugu. Primary
            language is determined from the
